...
def _open_meteo_daily_results_from_payload(
    store: Store,
    payload: object,
    *,
    days: Sequence[date],
    ...
    if not isinstance(payload, dict):
        raise RuntimeError(f"Invalid Open-Meteo payload for {store.name}: {payload!r}")
...
def fetch_open_meteo_forecast_week(
    stores: Sequence[Store],
    *,
    week_start: date,
    week_days: int,
    ...
    for group in chunked(list(stores), batch_size):
        ...
        payload = request_json_with_retry(http, OPEN_METEO_FORECAST_URL, params=params, timeout=60, max_attempts=8)
        weather_items = payload if isinstance(payload, list) else [payload]
        if len(weather_items) != len(group):
            raise RuntimeError(f"Open-Meteo Forecast returned {len(weather_items)} location payloads for {len(group)} stores")
        for store, item in zip(group, weather_items):
            results.extend(
                _open_meteo_daily_results_from_payload(
                    store,
                    item,
                    days=days,
...
def fetch_open_meteo_single_run_week(
    ...
    *,
    ...
    for group in chunked(list(stores), batch_size):
        ...
        payload = request_json_with_retry(http, OPEN_METEO_SINGLE_RUN_URL, params=params, timeout=60, max_attempts=8)
        weather_items = payload if isinstance(payload, list) else [payload]
        if len(weather_items) != len(group):
